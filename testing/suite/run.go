...
func (t *TestRun) Start() {
	...
	log.Println("================")
	log.Printf("expected server host [%v]", t.ServerHost)
...
func makeClient(clientName string, t *TestRun) PackageIndexerClient {
	client, err := MakeTCPPackageIndexClient(clientName, t.ServerHost, t.ServerPort)
	if err != nil {
		t.Failf("Error opening client to [%v:%d]: %v", t.ServerHost, t.ServerPort, err)
	}
	return client
}
